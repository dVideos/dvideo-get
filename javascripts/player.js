...
shortTermGw = "https://video.dvideo.io"
player = null
itLoaded = false
timeout = 1000
...
function createPlayer(posterHash, autoplay, branding, qualities, sprite, duration, subtitles) {
    ...
    c.poster = 'https://go.dvideo.io/ipfs/'+posterHash;
    c.controls = true;
    c.autoplay = autoplay;
    c.id = "player";
    ...
    player.brand({
        ...
        title: "Watch on dVideo",
        destination: "http://dvideo.io/#!/v/" + videoAuthor + '/' + videoPermlink,
...
function createLiveStream(autoplay, branding, content) {
    ...
    if (content) {
        ...
        request.open('GET', 'https://stream.dvideo.io:3000/oldStream/'+content.author+'/'+unix_timestamp, true);
        request.onload = function() {
            if (request.status >= 200 && request.status < 400) {
                ...
                if (d0 && d0 < 5*60 && dn > 6*60*60) {
                    ...
                    videosrc = 'https://video.dvideo.io/streams/'+data[0][0].filePath
                    player.src(videosrc)
                    return
                }
                var livesrc = 'https://stream.dvideo.io:4433/hls/normal%2b'+videoAuthor+'/index.m3u8'
                player.src({
                    src: livesrc,
